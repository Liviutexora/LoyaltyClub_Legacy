...
<div class="card mb-3">
    <div class="card-header">
        ...
        <div class="col-6 col-sm-auto d-flex align-items-center pr-0">
            <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0"><?=$this->lang->line('Admin Section Page Domains Page Title')?></h5>
        </div>
    ...
    <div class="card-body px-0 pt-0  card-datables">
        ...
        <button class="btn btn-warning mr-1 mb-1 float-right open-modal" type="button" ajaxlink="<?=site_url('add-domain')?>"><?=$this->lang->line('Admin Section Domains Page Label Add Domain')?></button>
        ...
        <table class="search-table table table-sm table-dashboard fs--1 datatable-table">
            ...
            <tr>
                <td input-placeholder="<?=$this->lang->line('Admin Section Page Domains Label Search By Name')?>"></td>                                
                <td input-placeholder="<?=$this->lang->line('Admin Section Page Domains Label Search By Order')?>"></td>
                <td input-placeholder="<?=$this->lang->line('Admin Section Page Domains Label Search By Status')?>"></td>
            </tr>
            </tbody>
        </table>
        <table class="table table-sm table-dashboard fs--1 datatable-table domains-table border-bottom" width="100%" data-language-label='{"Datatables Label Display" : "<?=$this->lang->line('Datatables Label Display')?>","Datatables Label Per Page" : "<?=$this->lang->line('Datatables Label Per Page')?>","Datatables Label No Data" : "<?=$this->lang->line('Datatables Label No Data')?>","Datatables Label Page" : "<?=$this->lang->line('Datatables Label Page')?>","Datatables Label Of" : "<?=$this->lang->line('Datatables Label Of')?>","Datatables Label No Records Available" : "<?=$this->lang->line('Datatables Label No Records Available')?>","Datatables Label Total Records" : "<?=$this->lang->line('Datatables Label Total Records')?>","Datatables Label Next" : "<?=$this->lang->line('Datatables Label Next')?>","Datatables Label Previous" : "<?=$this->lang->line('Datatables Label Previous')?>" }' data-options='{"responsive":false,"pagingType":"simple","lengthChange":false,"searching":false,"pageLength":11}'>
            <thead class="bg-200 text-900">
            <tr>
                <th class="sort pr-1 align-middle"><?=$this->lang->line('Admin Section Page Domains Label Domain Name')?></th>
                <th class="sort pr-1 align-middle"><?=$this->lang->line('Admin Section Page Domains Label Domain Order')?></th>
                <th class="sort pr-1 align-middle"><?=$this->lang->line('Admin Section Page Domains Label Domain Status')?></th>
...
$(document).ready(function(){
            if($('.domains-table').length){
                ...
                languageLabels = $('.domains-table').attr("data-language-label");
            ...
            $(document).on('click','#change-domain-status',function(event) {
                content = $(this).attr("lang-content");
                yes = $(this).attr("lang-yes");
                no = $(this).attr("lang-no");
                url = $(this).attr("url");
                id = $(this).attr("domainId");
                domainStatus = $(this).attr("domainStatus");
                var dialog = bootbox.dialog({
                        message: content,
                        closeButton: false,
                        buttons: {
